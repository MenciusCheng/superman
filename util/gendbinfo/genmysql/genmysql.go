...
func (m *mysqlModel) getPackageInfo(orm MySqlDB, info *model.DBInfo) {
	...
	for tabName, notes := range tabls {
		...
		if config.GetIsOutSQL() {
			...
			rows, err := orm.RawRows("show create table " + assemblyTable(tabName))
			//defer rows.Close()
			if err == nil {
				if rows.Next() {
...
func (m *mysqlModel) getTableElement(orm MySqlDB, tab string) (el []model.ColumnsInfo) {
	keyNameCount := make(map[string]int)
	KeyColumnMp := make(map[string][]keys)
	// get keys
	var Keys []keys
	orm.RawScan(&Keys, "show keys from "+assemblyTable(tab))
	for _, v := range Keys {
	...
	orm.RawScan(&list, "show FULL COLUMNS from "+assemblyTable(tab))
	...
	if config.GetIsForeignKey() {
		...
		orm.RawScan(&foreignKeyList, sql)
	}
	// ------------------end
...
func (m *mysqlModel) getTables(orm MySqlDB) map[string]string {
	...
	if m.GetOriginTableNames() != "" {
		...
		rows, err := orm.RawRows("show tables")
		if err != nil {
			if !config.GetIsGUI() {
				fmt.Println(err)
			}
	...
	var rows1 *sql.Rows
	if m.GetTableNames() != "" {
		rows1, err = orm.RawRows("SELECT TABLE_NAME,TABLE_COMMENT FROM information_schema.TABLES WHERE table_schema= '" + m.GetDbName() + "'and TABLE_NAME IN(" + m.GetTableNames() + ")")
		...
		rows1, err = orm.RawRows("SELECT TABLE_NAME,TABLE_COMMENT FROM information_schema.TABLES WHERE table_schema= '" + m.GetDbName() + "'")
	}
